...
export class CrearCompraNacionalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get(environment.base_url + '/php/inventario_fisico_puntos/lista_punto_funcionario', { params: { id: this.user } }).subscribe((res: any) => {
  ...
  filtros() {
    ...
    if (![0, ''].includes(this.selectedCategory.categoria + this.selectedCategory.subcategoria)) {
  ...
  // Si no se especifica un número de página, se cargará la ultima.
  paginarProductos(page = Math.ceil(this.listaProductosPorAgregar.length / this.pagination.pageSize)) {
    this.pagination.collectionSize = this.listaProductosPorAgregar.length;
    this.pagination.page = page;
    let offset = (page - 1) * this.pagination.pageSize;
    let productsPerPage = this.formCompra.value.Productos.slice(offset).slice(0, this.pagination.pageSize).map((item) => item = item.Id_Producto);
    this.listaProductosPorAgregar.map((item) => item.mostrarProductoEnTabla = (productsPerPage.includes(item.Id_Producto)) ? true : false);
  }
