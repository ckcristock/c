...
export class NotascreditocrearComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get(environment.base_url + '/php/lista_generales.php', { params: { modulo: 'Causal_No_Conforme' } }).subscribe((data: any) => {
      this.Motivos = data;
    });
  }
  ...
  BuscarClientesTerceros(tipo: any = false) {
    ...
    this.http.get(environment.base_url + '/php/clientes/get_terceros_por_tipo.php', { params }).subscribe((data: any) => {
      this.Cliente = data;
    });
  ...
  BuscarFactura(codigo) {
    ...
    if (this.Tipo_Factura_Selected != "") {
      if (Object.keys(this.DatosCliente).length > 0 && this.DatosCliente.Id_Cliente != undefined) {
        ...
        this.http.get(environment.base_url + '/php/notas_credito_nuevo/lista_facturas_cliente_notas_credito.php',
          { params })
          .subscribe((data: any) => {
