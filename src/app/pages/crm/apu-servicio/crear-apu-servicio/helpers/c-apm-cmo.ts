...
export const cApmCalculateLaborHelper = {
  createFillInCApm(form: FormGroup, fb: FormBuilder, data, profiles, cities) {
    if (data?.accompaniments_c) {
      data?.accompaniments_c?.forEach((r) => {
        let list = form?.get('c_apm_calculate_labor') as FormArray;
        let group = fb?.group({
        ...
        r?.travel_estimation_accompaniment_c?.forEach(v => {
          let travel = fb?.group({
  ...
  subtotalTravelExpense(list: FormArray, form: FormGroup) {
    setTimeout(() => {
      ...
      form?.patchValue({ subtotal_travel_expense_apm_c: total })
    }, 100);
  },
};
