...
export const cMeCalculateLaborHelper = {
  createFillInCMe(form: FormGroup, fb: FormBuilder, data, profiles, cities) {
    if (data.assemblies_start_up_c) {
      data.assemblies_start_up_c.forEach((r) => {
        let list = form.get('c_me_calculate_labor') as FormArray;
        let group = fb.group({
        ...
        list.push(group);
        this.subscribeCMe(group, form, list, profiles);
        r.travel_estimation_assemblies_start_up_c.forEach(v => {
          let travel = fb.group({
  ...
  subscribeCMe(group: FormGroup, form: FormGroup, list: FormArray, profiles: Array<any>) {
  ...
  subtotalTravelExpense(list: FormArray, form: FormGroup) {
    setTimeout(() => {
      ...
      form.patchValue({ subtotal_travel_expense_me_c: total })
    }, 100);
  },
};
