...
export class InventarioEstibasComponent implements OnInit {
  ...
  ConsultaCodigo(codigo) {
    ...
    if (!resultado) {
      this.GetProductoEstiba(params).subscribe((data: any) => {
  ...
  reVerificarSiExisteEnFila(producto, lote) {
    ...
    if (this.Productos[producto].Lotes.length > 1) {
      ...
      //recorremos los lotes menos el último que es el de insertar buscando coincidencias si la encuentra mande un text de error!
  ...
  reGuardarLotes() {
    ...
    datos.append("Id_Doc_Inventario_Fisico", this.Model["Id_Doc_Inventario_Fisico"]);
    datos.append("Productos", productos);
    this.saveLote(datos).subscribe((data: any) => {
      if (data.codigo == 'success') {
        this._swalService.show(data);
        this.Cargado = false;
